"""
UNet3D Model Explanation:
--------------------------
The UNet3D model implements a 3D U-Net architecture employing an
encoder-decoder structure with skip connections to recover spatial
information lost during downsampling.

Encoder Path (DownBlocks):
    - The encoder consists of multiple DownBlock modules.
    - Each DownBlock performs two convolutions followed by activations
      (and optional normalization) to extract features.
    - A pooling operation is applied (except at the deepest level) to
      progressively reduce spatial dimensions while increasing the feature depth.
    - Intermediate feature maps (before pooling) are stored for use in the decoder path.

Decoder Path (UpBlocks):
    - The decoder utilizes UpBlock modules to sequentially upsample the features.
    - Each UpBlock upsamples the feature maps using either transposed convolution
      or interpolation, then applies additional convolutions to refine the details.
    - Feature maps from the corresponding encoder stage are cropped (if necessary)
      and concatenated with the upsampled features to provide fine-grained context.
      
Final Convolution:
    - A final convolution layer maps the refined features onto the desired number
      of output channels, producing the segmentation output.

Additional Details:
    - The model supports both 2D and 3D operations based on the provided dimension parameter.
    - Customizable options include the activation function, normalization layer, convolution mode,
      and upsampling method.
    - This architecture is particularly useful for tasks involving volumetric data such as medical imaging
      or multi-temporal satellite data.
